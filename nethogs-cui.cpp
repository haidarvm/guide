 ...
 *
static void mvaddstr_truncate_cmdline(int row, int col, const char *name,
                                      const char *cmdline,
                                      std::size_t width) {
  if (showBasename) {
...
void Line::show(int row, unsigned int /*proglen*/, unsigned int /*devlen*/) {
    ...
    // Start positions
    const int column_offset_pid = 0;
    const int column_offset_program = column_offset_pid + 6;
    const int column_offset_sent = column_offset_program + 16;
    const int column_offset_received = column_offset_sent + 9;
    const int column_offset_unit = column_offset_received + 7;

    // PID column
    if (m_pid == 0)
        //mvaddch(row, column_offset_pid + COLUMN_WIDTH_PID - 1, i'?');
        mvaddch(row, column_offset_pid + 3, '?');
    else
        mvprintw(row, column_offset_pid, "%4d", m_pid);
    ...
    if (pos != std::string::npos)
     ...
     if (prog.size() > 15)
        prog = prog.substr(0, 14) + "…";

    //mvaddstr_truncate_cmdline(row, column_offset_program, prog.c_str());
    mvaddstr(row, column_offset_program, prog.c_str());
...
void Line::log() {
    ...
    if (!header_printed) {
        std::cout << std::left
                  << std::setw(8)  << "PID"
                  << std::setw(20) << "Program"
                  << std::setw(12) << "Sent(KB/s)"
                  << std::setw(12) << "Recv(KB/s)"
                  << std::endl;
        header_printed = true;
    }

    // Extract only the basename (strip directory path)
    std::string prog = m_name;
    size_t pos = prog.find_last_of('/');
    if (pos != std::string::npos)
        prog = prog.substr(pos + 1);

    // Truncate to avoid wide overflow (limit to 18 chars)
    if (prog.size() > 18)
        prog = prog.substr(0, 17) + "…";
...
// Adjust program column width: remove USER and DEV spacing
//proglen = cols - 20;
proglen = 20;
erase();
mvprintw(0, 0, "%s", caption->c_str());
...
mvprintw(2, 0,
	"PID  %-*.*s  %6s  %6s",
         proglen, proglen, "PROGRAM", "SENT", "RECV");
  ...
  mvprintw(rows - 1, 0, " TOTAL ");
mvprintw(rows - 1, cols - 5, "%7.3f %8.3f", sent_global, recv_global);
  attroff(A_REVERSE);
